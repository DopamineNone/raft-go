...
type config struct {
	...
	nctrlers      int
	ctrlerservers []*shardctrler.ShardController
...
func (cfg *config) StartServer(gi int, i int) {
	...
	gg.servers[i] = StartKVServer(ends, i, gg.saved[i], cfg.maxraftstate,
		gg.gid, mends,
...
func (cfg *config) StartCtrlerserver(i int) {
	...
	cfg.ctrlerservers[i] = shardctrler.StartShardController(ends, i, p)
...
func make_config(t *testing.T, n int, unreliable bool, maxraftstate int) *config {
	...
	// controler
	cfg.nctrlers = 3
	cfg.ctrlerservers = make([]*shardctrler.ShardController, cfg.nctrlers)
	for i := 0; i < cfg.nctrlers; i++ {
